...
LOCKFILE="/var/lock/aif_dyndns_ipv6_open.lock"

ARGSFILE="/var/tmp/aif_dyndns_ipv6_open.args"
  ...
  cat "$1" | while read rule; do
    ...
    if [ $? -ne 0 -a $cnt -gt 0 ]; then
      ...
      while [ $end_cnt -gt $cnt ]; do
        ip6tables -D DYNDNS_CHAIN $end_cnt
        end_cnt=$((end_cnt-1))
      done
      break
    fi
...
log_msg()
{
  logger -t "firewall: dyndns-ipv6-open" -p kern.info "$1"
  echo "$1" >&2
}
...
start)
  if [ -z "$1" -o -z "$2" ]; then
    echo "Usage: dyndns-ipv6-open-helper start ip6tables_path time rules_file"
    exit 1
  fi
  start_run "$@"
  ;;
...
*)
  echo "Usage: dyndns-ipv6-open-helper start|stop|status ip6tables_path"
