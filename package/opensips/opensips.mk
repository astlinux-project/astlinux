...
define OPENSIPS_INSTALL_TARGET_CMDS
	$(INSTALL) -D -m 0755 $(@D)/opensips $(TARGET_DIR)/usr/sbin/opensips
	# Install modules
	(for i in `find $(@D)/modules -name \*.so`; \
	do $(INSTALL) -D -m 0755 "$$i" $(TARGET_DIR)/usr/lib/opensips/modules/`basename "$$i"`; done)
	# Install opensipsctl
	$(INSTALL) -D -m 0644 $(@D)/scripts/opensipsctl.base $(TARGET_DIR)/usr/lib/opensips/opensipsctl/opensipsctl.base
	$(INSTALL) -D -m 0644 $(@D)/scripts/opensipsctl.ctlbase $(TARGET_DIR)/usr/lib/opensips/opensipsctl/opensipsctl.ctlbase
	$(INSTALL) -D -m 0644 $(@D)/scripts/opensipsctl.fifo $(TARGET_DIR)/usr/lib/opensips/opensipsctl/opensipsctl.fifo
	$(SED) 's:/usr/local/etc/opensips:/etc/opensips:g' \
		$(TARGET_DIR)/usr/lib/opensips/opensipsctl/opensipsctl.*
	...
	$(INSTALL) -D -m 0644 $(@D)/scripts/opensipsctlrc $(TARGET_DIR)/stat/etc/opensips/opensipsctlrc
	$(INSTALL) -D -m 0644 package/opensips/opensips.cfg $(TARGET_DIR)/stat/etc/opensips/opensips.cfg
